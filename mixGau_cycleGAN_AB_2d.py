...
class CycleGANModel():
	...
	def sample_data(self):
		...
		b_sampler = Sampler(mean_B, std_B, num_B, 4).mixgaussian()
	...
	def backward_G(self):
		...
		self.loss_G_AB = self.criterionGAN(pred_fake, True)		
		...
		# D_AB(G_A(B))
		pred_fake = self.netD_AB.forward(self.fake_A)
		self.loss_G_BA = self.criterionGAN(pred_fake, False)
		...
		#pb = 0.5 * np.maximum(np.minimum(1, -(np.log2(c_epoch/32000))/4), 0)	
		pb = 0.25 * (- np.maximum(np.minimum(1, -(np.log2(c_epoch/epoch))/4), 0) + 1) #divided by satuate epoch
		#pb = -(4/epoch ** 2) * c_epoch + (4/epoch) * c_epoch

		# combined loss
		self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B + self.loss_G_AB * pb + self.loss_G_BA * pb
	...
	def get_current_errors(self):
		...
		G_AB = self.loss_G_AB.data[0]
		G_BA = self.loss_G_BA.data[0]

		return OrderedDict([('D_A', D_A), ('G_A', G_A), ('Cyc_A', Cyc_A), ('G_AB', G_AB),
	                        ('D_B', D_B), ('G_B', G_B), ('Cyc_B', Cyc_B), ('G_BA', G_BA)])
	...
	def save_fig(self, epoch):
		...
		outDir = "./DomainB/mixGau-cycle-d_AB-6/"
		try:
			os.makedirs(outDir)
		except OSError:
			pass
		...
		outDir = "./DomainA/mixGau-cycle-d_AB-6/"
		try:
			os.makedirs(outDir)
		except OSError:
			pass
...
class preTrain():
	def __init__(self, data_A, data_B):
		self.netD_AB = Discriminator(input_size = d_input_size, hidden_size = d_hidden_size, output_size = d_output_size)
		self.preCriterion = GANLoss()
		self.optimizer_D_AB = torch.optim.Adam(self.netD_AB.parameters(), lr = d_learning_rate, betas = optim_betas, weight_decay = l2)
		self.data_A = data_A
		self.data_B = data_B

		print('---------- pre_train initialized -------------')

	def set_input(self, real_A, real_B):
		self.input_A = torch.Tensor(np.random.choice(self.data_A, (minibatch_size, 1)))
		self.input_B = torch.Tensor(np.random.choice(self.data_B, (minibatch_size, 1)))
	...
	def backward_D(self):
		predict_A = self.netD_AB(self.real_A)
		loss_D_A = self.preCriterion(predict_A, False)

		predict_B = self.netD_AB(self.real_B)
		loss_D_B = self.preCriterion(predict_B, True)

		self.loss_D = (loss_D_A + loss_D_B) * 0.5
	...
	def get_current_errors(self):
		return OrderedDict([('D_AB', self.loss_D.data[0])])
...
pre_model = preTrain(model.data_A, model.data_B)
pre_epoch = 150
for pre_i in range(0, pre_epoch):
	np.random.shuffle(pre_model.data_A)
	np.random.shuffle(pre_model.data_B)

	dataset_A = np.split(pre_model.data_A, len(pre_model.data_A) / minibatch_size)
	dataset_B = np.split(pre_model.data_B, len(pre_model.data_B) / minibatch_size)

	for pre_j in range(0, np.minimum(len(dataset_A), len(dataset_B))):
		pre_model.set_input(dataset_A[pre_j], dataset_B[pre_j])
		pre_model.optimize_parameters()
	
	print(pre_model.get_current_errors())

model.netD_AB = pre_model.netD_AB
