...
class CycleGANModel():
	def __init__(self):
		...
		self.netD_BA = Discriminator(input_size = d_input_size, hidden_size = d_hidden_size, output_size = d_output_size)
		...
		self.optimizer_D_A = torch.optim.Adam(itertools.chain(self.netD_A.parameters(), self.netD_AB.parameters()), lr = d_learning_rate, betas = optim_betas, weight_decay = l2)
		self.optimizer_D_B = torch.optim.Adam(itertools.chain(self.netD_B.parameters(), self.netD_BA.parameters()), lr = d_learning_rate, betas = optim_betas, weight_decay = l2)
	...
	def backward_D_basic(self, netD, netD2, real1, fake, real2):
		# Real
		pred_real = netD.forward(real1)
		...
		# Real
		pred_real2 = netD2.forward(real2)
		loss_D2_real = self.criterionGAN(pred_real2, True)
		# Fake
		pred_fake2 = netD2.forward(fake.detach())
		loss_D2_fake = self.criterionGAN(pred_fake2, False)
		# Combined loss
		loss_D2 = (loss_D2_real + loss_D2_fake) * 0.5
		# backward
		loss_D2.backward()

		return loss_D, loss_D2

	def backward_D_A(self):
		fake_B = self.netG_A.forward(self.real_A) 
		self.loss_D_A, self.loss_D_AB = self.backward_D_basic(self.netD_A, self.netD_AB, self.real_B, fake_B, self.real_A)

	def backward_D_B(self):
		fake_A = self.netG_B.forward(self.real_B)
		self.loss_D_B, self.loss_D_BA = self.backward_D_basic(self.netD_B, self.netD_BA, self.real_A, fake_A, self.real_B)
	...
	def backward_G(self):
		...
		self.loss_G_AB = self.criterionGAN(pred_fake, False)

        # D_BA(G_B(B))
		pred_fake = self.netD_BA.forward(self.fake_A)
		...
		# combined loss
		self.loss_G = self.loss_G_A + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + d2_rate * self.loss_G_AB + d2_rate * self.loss_G_BA
	...
	def save_fig(self, epoch):
		...
		outDir = "./DomainB/mixGau-cycle-d2-1/"
		try:
			os.makedirs(outDir)
		except OSError:
			pass
		...
		outDir = "./DomainA/mixGau-cycle-d2-1/"
		try:
			os.makedirs(outDir)
		except OSError:
			pass
