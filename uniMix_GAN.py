...
class Sampler():
	...
	def uniform(self, low, upper):
		return lambda m, n: np.random.uniform(low, upper, m*n)
...
d_sampler = Sampler(mean_A, std_A, num_A).mixgaussian()
g_sampler = Sampler(mean_B, std_B, num_B).uniform(1, 3)
...
for epoch in range(num_epochs):
	...
	d_real_loss = criterion(d_real_decision, Variable(torch.ones(minibatch_size, 1)))  # ones = true
	...
	d_fake_loss = criterion(d_fake_decision, Variable(torch.zeros(minibatch_size, 1)))  # zeros = fake
	...
	g_loss = criterion(dg_fake_decision, Variable(torch.ones((minibatch_size, 1))))  # we want to fool, so pretend it's all genuine
	...
	if epoch % save_epoch == 0:
		...
		outDir = "/Users/maureen/Documents/Python/simpleGAN/GAN/uniGau/" + str(epoch) +".png"
		plt.savefig(outDir)
